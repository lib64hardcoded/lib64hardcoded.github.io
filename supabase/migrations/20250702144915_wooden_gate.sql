...
-- Instead of using jsonb_set, we'll use a more compatible approach
DO $$
DECLARE
  ...
  LOOP
    ...
    FOR i IN 0..jsonb_array_length(report_record.comments)-1 LOOP
      ...
      IF comment_item->>'author' LIKE '%Admin%' THEN
        comment_item := comment_item || '{"author_grade": "Admin"}'::jsonb;
      ELSIF comment_item->>'author' LIKE '%Support%' THEN
        comment_item := comment_item || '{"author_grade": "Support"}'::jsonb;
      ELSIF comment_item->>'author' LIKE '%V5%' THEN
        comment_item := comment_item || '{"author_grade": "V5"}'::jsonb;
      ELSE
        comment_item := comment_item || '{"author_grade": "V4"}'::jsonb;
      END IF;
